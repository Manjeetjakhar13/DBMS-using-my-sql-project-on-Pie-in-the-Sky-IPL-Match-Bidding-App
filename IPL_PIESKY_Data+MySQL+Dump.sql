...
select bidder_id,bidder_name,won,total,(won/total)*100 percent from
(select bidder_id ,bidder_name,(select count(bid_status) from ipl_bidding_details d where d.bidder_id=c.bidder_id and bid_status='won' ) won,
(select no_of_matches from ipl_bidder_points s where s.bidder_id=c.bidder_id) total from ipl_bidder_details c group by bidder_id) t order by percent desc;

#⦁	Display the number of matches conducted at each stadium with stadium name, city from the database.
select stadium_name,city,count(mc.stadium_id) no_of_match from ipl_stadium s left join ipl_match_schedule mc on s.stadium_id=mc.stadium_id 
...
select stadium_id,name,won,total_played,round((won/total_played)*100,2) winpercent from
(select stadium_id,(select stadium_name from ipl_stadium m where m.stadium_id=s.stadium_id) name, count(*) won,
(select count(*) from ipl_match_schedule a where s.stadium_id=a.stadium_id ) total_played from ipl_match_schedule  s
where match_id in (select match_id from ipl_match where match_winner=toss_winner ) group by stadium_id order by stadium_id)s ;

#⦁	Show the total bids along with bid team and team name.
select bid_team,(select team_name from ipl_team it where it.team_id=bid_team) ,sum(bids) totalbids from
(select bid_team,(select no_of_bids from ipl_bidder_points bp where bp.bidder_id=bd.bidder_id )bids from ipl_bidding_details bd)t group by bid_team order by bid_team ;
#⦁	Show the team id who won the match as per the win details.
select *,(select team_name from ipl_team it where  winner_team_id=it.team_id ) team_name from
(select win_details,if(match_winner=1,team_id1,team_id2) winner_team_id from ipl_match im)t;

#⦁	Display total matches played, total matches won and total matches lost by team along with its team name.
select (select team_name from ipl_team t where t.team_id=ts.team_id) team_name,matches_played,matches_won,matches_lost from ipl_team_standings ts order by tournmt_id;
#⦁	Display the bowlers for Mumbai Indians team.
select (select player_name from ipl_player ip where ip.player_id=itp.player_id) playername,player_role,team_id from ipl_team_players itp
...
select * from
(select team_id,(select team_name from ipl_team it where it.team_id=itp.team_id )teamname ,count(*) totalallrounder from ipl_team_players itp 
where player_role='all-rounder' group by team_id) t where totalallrounder >4 order by totalallrounder desc;
